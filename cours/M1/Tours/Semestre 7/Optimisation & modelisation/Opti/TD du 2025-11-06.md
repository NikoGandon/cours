...
On a une variable d'intervalle $T_{i}$ par tâche

## Domaine

$\forall i, D(\text{proc}(T_{i}))=\{ p_{i} \}$
$\forall i, D(\text{start}(T_{i}))=\{ r_{i},\dots, \text{UB}-p_{i} \}$
$\forall i, D(\text{end}(T_{i}))=\{ r_{i}+p_{i},\dots , \text{UB}\}$
## Contrainte

$\text{Disjunctive}(\{ T_{i} \})$ (ou $\text{noOverlap}$)

## Objectif

$\min \max \text{end}(T_{i})-d_{i}$
 
# Exercice 5

Soit $X=\{ X,Y,Z \}$, $D(X)=\{ 2,3,4 \}, \quad D(Y)= \{ 0,\dots,4 \},\quad D(Z)= \{ 0,\dots,3 \}$, $Y \leq 3, \quad X \leq Y+1, \quad Y=X+Z$

8. Est-ce noeud consistant ?
   - Noeud consistant = 1-consistant
   - Il faut regarder les contraintes unaires,
   - Il n'y en a qu'un $Y\leq 3$
   - On peut enlever $4$ du domaine de $Y$ $\implies D(Y)=\{ 0,\dots,3 \}$
9. Arc-consistant: on regarde 2 variables à la fois
   Ici, on regarde $X\leq Y+1$
   $\implies D(X)=\{ 2,3,4 \}\quad D(Y)=\{ \cancel{0},\cancel{1},\dots,3 \}$
   Pour la chemin consistance, 
   On regarde
10. 